...
class AdminMiddleware
{
     ...
     * and issues a new access token. The token is then validated and the user role is checked.
     * Only if the role is "admin" will the request proceed. Otherwise, an unauthorized error is returned.
     *
    ...
    public function handle(Request $request, Closure $next)
    {
        ...
        if (!$accessToken && $refreshToken) {
            try {
                ...
                // Attach the new token to the request header.
        ...
        if (!isset($claims->user_role) || strtolower($claims->user_role) !== 'admin') {
